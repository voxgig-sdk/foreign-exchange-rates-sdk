# foreign-exchange-rates-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ForeignExchangeRates SDK as
two agent tools — `foreign-exchange-rates_list` and `foreign-exchange-rates_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/foreign-exchange-rates-mcp)
make build
...
claude mcp add --scope user foreign-exchange-rates \
  -- /absolute/path/to/foreign-exchange-rates-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./foreign-exchange-rates-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/foreign-exchange-rates-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user foreign-exchange-rates \
     -- "$PWD"/dist/*/foreign-exchange-rates-mcp -transport stdio
   ```
...
### Call the `foreign-exchange-rates_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `foreign-exchange-rates_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `foreign-exchange-rates_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./foreign-exchange-rates-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"foreign-exchange-rates_load","arguments":{"entity":"account","query":{"id":1}}}}'
```

...
// ForeignExchangeRates SDK feature test
//
...
class FtCtrl extends ForeignExchangeRatesControl
{
...
class FtClient
{
    ...
    private ForeignExchangeRatesUtility $utility;

    public function __construct(ForeignExchangeRatesUtility $utility, array $options)
    {
    ...
    public function get_utility(): ForeignExchangeRatesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ForeignExchangeRatesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ForeignExchangeRatesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ForeignExchangeRatesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ForeignExchangeRatesConfig::make_feature($name);
    ...
    private static function buildUrl(ForeignExchangeRatesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ForeignExchangeRatesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ForeignExchangeRatesResponse($fetched);
        }
    ...
    private function fail(ForeignExchangeRatesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ForeignExchangeRatesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class ForeignExchangeRatesMakeError
{
    public static function call(?ForeignExchangeRatesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ForeignExchangeRatesContext([], null);
        }
        $op = $ctx->op ?? new ForeignExchangeRatesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ForeignExchangeRatesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ForeignExchangeRatesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ForeignExchangeRatesError('', $msg, $ctx);
        ...
        if ($err instanceof ForeignExchangeRatesError) {

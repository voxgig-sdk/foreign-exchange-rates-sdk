...
module ForeignExchangeRatesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ForeignExchangeRatesResponse.new({})
    ...
    if fetch_err
      ...
      response = ForeignExchangeRatesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ForeignExchangeRatesResponse.new(fetched)
    else

# foreign-exchange-rates-cli

boru-driven command-line client **and** interactive REPL for the ForeignExchangeRates
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/foreign-exchange-rates-cli)
make build

# 2. See usage (words, entities, env vars)
./foreign-exchange-rates-cli --help

# 3. Provide credentials once, via the environment
export FOREIGN_EXCHANGE_RATES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./foreign-exchange-rates-cli load 1 account            # {id:1} shorthand
./foreign-exchange-rates-cli load '{id:1}' account       # explicit match map

# 5. Override the API base URL for a single call
FOREIGN_EXCHANGE_RATES_BASE=https://api.example.com ./foreign-exchange-rates-cli load 1 account

# 6. No arguments -> interactive REPL
./foreign-exchange-rates-cli
foreign-exchange-rates> load 1 account
foreign-exchange-rates> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/foreign-exchange-rates-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/foreign-exchange-rates-cli load 1 account
   ```
...
```sh
./foreign-exchange-rates-cli load 1 account          # scalar shorthand for {id:1}
./foreign-exchange-rates-cli load '{id:1}' account     # explicit match map
```
...
```sh
export FOREIGN_EXCHANGE_RATES_APIKEY=sk_live_xxx            # API key
export FOREIGN_EXCHANGE_RATES_BASE=https://api.example.com  # optional: override the API base URL
./foreign-exchange-rates-cli load 1 account
```
...
Run with no arguments to open a REPL (prompt `foreign-exchange-rates>`). Each line is
evaluated as its own boru expression:

```text
$ ./foreign-exchange-rates-cli
foreign-exchange-rates> load 1 account
foreign-exchange-rates> /help
foreign-exchange-rates> /quit
```
...
| Variable | Purpose |
|----------|---------|
| `FOREIGN_EXCHANGE_RATES_APIKEY` | API key sent with every request. |
| `FOREIGN_EXCHANGE_RATES_BASE` | Optional override of the API base URL. |
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/foreign-exchange-rates-cli`. |
...
account convert currency range rate
...
(`./foreign-exchange-rates-cli <expr>`) and interactively (the REPL), and expressions compose the

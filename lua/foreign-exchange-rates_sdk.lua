...
require("foreign-exchange-rates_types")
...
local ForeignExchangeRatesSDK = {}
ForeignExchangeRatesSDK.__index = ForeignExchangeRatesSDK
...
ForeignExchangeRatesSDK._make_feature = _make_feature


function ForeignExchangeRatesSDK.new(options)
  local self = setmetatable({}, ForeignExchangeRatesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ForeignExchangeRatesSDK:options_map()
...
function ForeignExchangeRatesSDK:get_utility()
  return Utility.copy(self._utility)
end


function ForeignExchangeRatesSDK:get_root_ctx()
  return self._rootctx
end


function ForeignExchangeRatesSDK:prepare(fetchargs)
...
function ForeignExchangeRatesSDK:direct(fetchargs)
...
function ForeignExchangeRatesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ForeignExchangeRatesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ForeignExchangeRatesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ForeignExchangeRatesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ForeignExchangeRatesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ForeignExchangeRatesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ForeignExchangeRatesSDK:Account(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ForeignExchangeRatesSDK:Convert(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ForeignExchangeRatesSDK:Currency(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ForeignExchangeRatesSDK:Range(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ForeignExchangeRatesSDK:Rate(data)
...
function ForeignExchangeRatesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ForeignExchangeRatesSDK.new(sdkopts)
  sdk.mode = "test"
...
return ForeignExchangeRatesSDK

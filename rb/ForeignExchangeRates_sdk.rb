...
require_relative 'ForeignExchangeRates_types'
...
class ForeignExchangeRatesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ForeignExchangeRatesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ForeignExchangeRatesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ForeignExchangeRatesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ForeignExchangeRatesError.new(
        "#{op}_allow",
        "ForeignExchangeRatesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ForeignExchangeRatesError => err
    ...
    if fetched.is_a?(Hash)
      status = ForeignExchangeRatesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ForeignExchangeRatesError.new(
        "graphql_error", "ForeignExchangeRatesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ForeignExchangeRatesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

...
class ConvertEntity
  ...
  def data_set(args)
    if args
      @_data = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ConvertListMatch, Hash, nil] match filter (any subset of
  #   Convert fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Convert>, Array] the matching Convert items; raises ForeignExchangeRatesError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Convert, Hash] the created Convert; raises ForeignExchangeRatesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ForeignExchangeRatesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

...
func TestConvertEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FOREIGNEXCHANGERATES_TEST_CONVERT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		convertRef01List, convertRef01ListOk := convertRef01ListResult.([]any)
		if !convertRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", convertRef01ListResult)
		}

		foundItem := vs.Select(entityListToData(convertRef01List), map[string]any{"id": convertRef01Data["id"]})
...
func convertBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FOREIGNEXCHANGERATES_TEST_CONVERT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FOREIGNEXCHANGERATES_TEST_CONVERT_ENTID"])

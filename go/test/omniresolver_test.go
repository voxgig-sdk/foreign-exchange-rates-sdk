...
import (
	...
	omni "github.com/voxgig-sdk/foreign-exchange-rates-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ForeignExchangeRatesError); is {
				out := map[string]any{
					"name":    "ForeignExchangeRatesError",
...
func omniCtx(arg any, client *sdk.ForeignExchangeRatesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ForeignExchangeRatesSDK); is {
			client = live

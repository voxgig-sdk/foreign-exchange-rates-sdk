...
func TestAccountEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FOREIGNEXCHANGERATES_TEST_ACCOUNT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func accountBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FOREIGNEXCHANGERATES_TEST_ACCOUNT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FOREIGNEXCHANGERATES_TEST_ACCOUNT_ENTID"])

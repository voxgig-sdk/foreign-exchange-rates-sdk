...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FOREIGNEXCHANGERATES_TEST_LIVE") == "TRUE" ||
		os.Getenv("FOREIGNEXCHANGERATES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FOREIGNEXCHANGERATES_TEST_EXPLAIN"); explain != "" {

...
const __1 = require("..");
const SDK_NAME = 'ForeignExchangeRatesSDK';
...
const TEST_SEED = { "entity": { "account": { "test01": { "id": "test01" } }, "convert": { "test01": { "id": "test01" } }, "currency": { "test01": { "id": "test01" } }, "range": { "test01": { "id": "test01" } }, "rate": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ForeignExchangeRatesSDK\b/.test(code) ||
        /\bForeignExchangeRatesSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ForeignExchangeRatesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ForeignExchangeRatesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

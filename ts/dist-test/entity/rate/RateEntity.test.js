...
(0, node_test_1.describe)('RateEntity', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('FOREIGN_EXCHANGE_RATES_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.ForeignExchangeRatesSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set FOREIGN_EXCHANGE_RATES_TEST_RATE_ENTID JSON to run live');
            return;
        }
        ...
        (0, node_assert_1.default)(rate_ref01_data_dt0.id === rate_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    let client = __1.ForeignExchangeRatesSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['rate01', 'rate02', 'rate03', 'v101', 'v102', 'v103'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['FOREIGN_EXCHANGE_RATES_TEST_RATE_ENTID'];
    ...
    idmap = env['FOREIGN_EXCHANGE_RATES_TEST_RATE_ENTID'];
    const live = 'TRUE' === env.FOREIGN_EXCHANGE_RATES_TEST_LIVE;
    if (live) {
        client = new __1.ForeignExchangeRatesSDK(merge([

# ForeignExchangeRates SDK test runner
...
class ForeignExchangeRatesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ForeignExchangeRatesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ForeignExchangeRatesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ForeignExchangeRatesTestRunner.getenv("FOREIGN_EXCHANGE_RATES_TEST_LIVE")
        override = ForeignExchangeRatesTestRunner.getenv("FOREIGN_EXCHANGE_RATES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ForeignExchangeRatesTestRunner.getenv(key)
        ...
        explain = ForeignExchangeRatesTestRunner.getenv("FOREIGN_EXCHANGE_RATES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ForeignExchangeRatesTestRunner._test_control is not None:
            return ForeignExchangeRatesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ForeignExchangeRatesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ForeignExchangeRatesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ForeignExchangeRatesTestRunner.load_test_control()
...
def load_env_local():
    ForeignExchangeRatesTestRunner.load_env_local()


def env_override(m):
    return ForeignExchangeRatesTestRunner.env_override(m)


def entity_data(v):
    return ForeignExchangeRatesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ForeignExchangeRatesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ForeignExchangeRatesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ForeignExchangeRatesTestRunner.load_test_control()


def live_client_options():
    return ForeignExchangeRatesTestRunner.live_client_options()


def live_delay_ms():
    return ForeignExchangeRatesTestRunner.live_delay_ms()

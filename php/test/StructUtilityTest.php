...
// ForeignExchangeRates SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ForeignExchangeRatesOmni::makeStructRunner(
    ...
    {
        return ForeignExchangeRatesOmni::objmodel(ForeignExchangeRatesOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ForeignExchangeRatesOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ForeignExchangeRatesOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ForeignExchangeRatesOmni::structfix($now, Struct::undef()))
                        !== json_encode(ForeignExchangeRatesOmni::structfix($val, Struct::undef()))) {

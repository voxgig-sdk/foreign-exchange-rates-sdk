...
//    only - a typed ForeignExchangeRatesContext there would make every ctx
...
final class ForeignExchangeRatesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ForeignExchangeRatesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ForeignExchangeRatesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ForeignExchangeRatesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ForeignExchangeRatesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ForeignExchangeRatesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ForeignExchangeRatesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ForeignExchangeRatesError(is_string($code) ? $code : '', $msg);
    }

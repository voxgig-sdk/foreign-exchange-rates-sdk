...
// ForeignExchangeRates SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ForeignExchangeRatesTestFeature extends ForeignExchangeRatesBaseFeature
{
    ...
    public function init(ForeignExchangeRatesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ForeignExchangeRatesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ForeignExchangeRatesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ForeignExchangeRatesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ForeignExchangeRatesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ForeignExchangeRatesParam::call($ctx, $k);

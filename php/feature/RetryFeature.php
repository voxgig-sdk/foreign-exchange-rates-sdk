...
// ForeignExchangeRates SDK retry feature
...
class ForeignExchangeRatesRetryFeature extends ForeignExchangeRatesBaseFeature
{
    ...
    public function init(ForeignExchangeRatesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ForeignExchangeRatesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ForeignExchangeRatesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ForeignExchangeRatesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
